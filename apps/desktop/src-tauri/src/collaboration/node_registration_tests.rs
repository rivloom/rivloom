...
fn durable_attempt_precedes_binding_and_neither_can_be_overwritten_on_restart() {
    ...
    assert_eq!(
        store.binding(&registration),
        Err(RegistrationError::Incomplete)
    );
    let reopened = RegistrationStore::new(store.directory.clone()).unwrap();
    assert_eq!(
        reopened.begin(&registration),
        Err(RegistrationError::Existing)
    );
    reopened.complete(&registration, &binding()).unwrap();
    assert_eq!(reopened.binding(&registration), Ok(binding()));
    assert_eq!(
        reopened.complete(&registration, &binding()),
        Err(RegistrationError::Existing)
    );
    let disk: serde_json::Value =
        serde_json::from_slice(&fs::read(store.directory.join("registration-v1.json")).unwrap())
            .unwrap();
    assert_eq!(
        disk,
        json!({"version":1,"identityId":"bob","deviceId":"bob-device","descriptor":registration.descriptor,"confirmedFingerprint":registration.confirmed_fingerprint})
    );
}
...
fn identity_binding_and_prior_fingerprint_must_match_without_implicit_reconfirmation() {
    ...
    for foreign in [
        ...
        assert_eq!(
            store.complete(&registration, &foreign),
    ...
    assert!(matches!(
        NodeRegistration::confirmed(
            &identity(),
            &registration.descriptor.encode().unwrap(),
            &"0".repeat(64)
        ),
        Err(RegistrationError::Invalid)
    ));
    ...
    fs::write(
        store.directory.join("registration-v1.json"),
        serde_json::to_vec(&changed).unwrap(),
    )
    .unwrap();
    assert_eq!(store.load(&identity()), Err(RegistrationError::Invalid));
    assert_eq!(
        store.complete(&registration, &binding()),
        Err(RegistrationError::Invalid)
    );
}
...
fn invalid_paths_or_descriptors_do_not_create_enrollment_storage() {
    assert!(matches!(
        RegistrationStore::new("relative/node".into()),
        Err(RegistrationError::Invalid)
    ));
    let temp = TempDir::new().unwrap();
    let store = RegistrationStore::new(temp.path().join("node-client")).unwrap();
