...
impl AppServerSupervisor {
    ...
    pub(super) fn set_notification_observer(&mut self, observer: Arc<dyn NotificationObserver>) {
        self.notification_observer = Some(observer.clone());
    ...
    fn activate_connection(
        ...
        if let Some(observer) = self.notification_observer.clone() {
            connection.set_notification_observer(observer);
        }
