...
- 平台：Windows
- 验证提交：`1c531ca9eb`
- 状态：R1、R2 本地实现与自动化验证完成；远程发布和真实 Runtime 原生 smoke 待完成
...
自动化 Gate 已通过，但还不能把 R2 写成“已发布并完成人工验收”：R1.1 之外的 stacked
分支尚未上传，真实已登录 Codex 的原生窗口 smoke 也没有在无人值守状态下调用。后者会
使用真实账号和模型，并可能等待本机审批，应在发布前由用户知情参与一次。
...
| R1.2 本地身份存储                | 完成     | 通过       | 本地 stacked 分支              |
| R1.3 双状态首页                  | 完成     | 通过       | 本地 stacked 分支              |
...
| R2.5 RunReceipt、编排、命令与 UI | 完成     | 通过       | 后半段仅在本地 stacked 分支    |
...
1. 按 stack 顺序发布 R1.2 到 R2 文档分支；R1.1 当前 PR 为
   `https://github.com/rivloom/rivloom/pull/41`。本地完成不冒充远端已发布。
