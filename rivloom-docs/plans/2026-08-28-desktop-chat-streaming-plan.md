...
1. Test stable `account/rateLimits/read`, sparse `account/rateLimits/updated`, explicit exhaustion, malformed windows, non-`codex` buckets, old connection identity, old account revision, late refresh responses, failed refresh staleness and every permitted or forbidden exhausted-state relaxation.
2. Borrow-project only App Server's top-level compatibility view `rateLimits`; never deserialize `rateLimitsByLimitId` or reset-credit rows. Define a fixed snapshot containing `available | exhausted | unknown`, 0–100 primary/secondary display percentages and reset seconds bounded to 0–253402300799. Drop bucket names and raw JSON immediately. Treat negative window usage and monthly remaining percentages outside 0–100 as malformed, clamp window usage at or above 100 as exhausted, and let any trusted exhaustion signal win over unrelated malformed fields. Discard an invalid optional reset field without weakening exhaustion.
3. Add a dedicated account revision separate from the existing request refresh revision; bump it synchronously on connection/account lifecycle changes and clear the old quota before spawning work. Route sparse notifications to `AccountService` first, then hand only its normalized snapshot to `ChatService`; never issue a synchronous request on the reader thread.
4. Allow command/background callers to refresh through `account/rateLimits/read`; bind every result to connection identity and account revision. Sparse updates merge field-by-field or tighten but never clear explicit exhaustion; only a valid same-account full read may establish `available`. A reached reset time may change `exhausted` only to `unknown`, and only when that exact saturated window was the sole exhaustion cause; spend-control, monthly-limit and reached-type exhaustion require a full read. Failed refreshes retain a stale same-revision snapshot; structurally malformed notifications do not overwrite it, while safely projected unknown semantics tighten availability to `unknown`. With no snapshot, show an unknown warning that permits only user-initiated sending and never triggers automatic sending or retry.
