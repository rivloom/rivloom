...
> 路线更新（2026-08-30）：本记录的 A2 验证结论仍有效，但原“完整 A3 Chat 是唯一
> 下一步”的安排已经被
> [Runtime Host 与协作闭环设计](2026-08-30-runtime-host-collaboration-design.md)取代。
...
下一步不再先建设完整 thread 恢复和 Chat 页面。当前路线只实现 Task Run 所需的有界
`turn/start`、事件、中断和 Artifact，然后进入两 Node 委派与审查闭环。
