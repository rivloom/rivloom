# Rivloom Desktop A2 本地项目与会话设计

- 状态：已实施（PR #23–#35，最终实现提交 `f85e075b8a0ac38694d458ba3015e6d1bbaf0c9a`）
- 日期：2026-08-27
...
最多累计 500 条 thread 摘要；服务端返回超过请求 limit 或游标超过 4 KiB 时拒绝该页并
保留已加载结果。稳定协议允许的超长 `name`、`preview` 在 UTF-8 字符边界截断为有界
展示字段，避免单条合法 thread 令整页不可用。
...
设计经 PR #22 评审接受，A2.1–A2.7 已通过 PR #23–#35 分阶段实施并合并。
