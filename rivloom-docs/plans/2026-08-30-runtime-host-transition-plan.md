...
| R3 最小 Brain      | 后端、托管及 Node 已合并；协作 UI 待合并 | 补齐能力声明/恢复与两机验收；Gate R3 未通过 |
| R4 远端委派        | 未开始                                       | 依赖 Gate R3                                        |
| R5 Artifact 审查   | 未开始                                       | 依赖 Gate R4                                        |
| R6 第二 Runtime    | 未开始                                       | 依赖 Gate R5，并先完成许可证与产品能力评审          |
| R7 Skill Directory | 未立项                                       | 依赖 Gate R5、R6 和两个 Runtime 的真实需求证据      |

这里的“本地完成”不等于已推送、已合并或已完成人工验收。精确分支、测试证据、安全审计和
...
本地核心已合入 `ab72fcf5ff` 并复验，分 PR 记录及限制见 [R3.3 记录](2026-08-31-runtime-host-r3-3-verification.md)。
...
协作界面接线、权限与浏览器检查见 [UI 验证记录](2026-08-31-r3-collaboration-ui-verification.md)。
