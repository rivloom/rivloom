...
截至 R3.4 收口，TLS 私钥由调用方注入，尚无桌面托管/命令和证书引导。
后续 [托管记录](2026-08-31-r3-desktop-hosting-verification.md) 补充本机 TLS 身份和命令接线；
Node/UI 接线、owner 恢复流程和两台真实 Windows 设备验收仍待完成。Gate R3 应先于 R4，不能提前开始远端执行。
